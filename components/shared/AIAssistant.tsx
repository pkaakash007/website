...
const RECOMMENDATIONS: Record<string, Recommendation> = {
  ...
  "build-tech": {
    ...
    description: "Next.js web applications, mobile apps, SaaS platform development, custom CRM/ERP integration, and cloud infrastructure architecture.",
    channels: ["Custom SaaS Architecture", "Next.js Web Platforms", "iOS & Android Apps", "Cloud & DevOps", "Enterprise API Integrations"],
    ctaUrl: "/technology",
...
export default function AIAssistant() {
  ...
  return (
    <>
      {/* Floating Trigger Button (Secondary, Sleek, Discrete) */}
      <div className="fixed bottom-6 right-6 z-40">
        <button
          onClick={() => setIsOpen(!isOpen)}
          aria-label="Real Result Assistant"
          className="flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-primary/95 hover:bg-black text-white border border-gold-400/40 shadow-card hover:shadow-gold-subtle transition-all duration-300 group"
        >
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-6 z-50 w-[92vw] sm:w-[420px] max-h-[85vh] bg-white rounded-3xl border border-border shadow-floating overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-200">
          {/* Header */}
              ...
              {QUICK_GOALS.map((g) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl transition-all duration-200 flex items-center justify-between text-xs font-semibold ${
                      isSelected
              ...
              <div className="p-4 rounded-2xl bg-canvas border border-gold/40 shadow-subtle space-y-3 animate-in fade-in duration-300">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[10px] font-mono text-muted hover:text-primary flex items-center gap-1"
                  >
                ...
                <div className="pt-2 flex items-center gap-2">
                  <Button
                    ...
                    className="text-xs py-2 px-4 w-full"
                  >
